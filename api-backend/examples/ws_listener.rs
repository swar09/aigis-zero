...
async fn main() -> anyhow::Result<()> {
    let ws_url =
        std::env::var("WS_URL").unwrap_or_else(|_| "ws://127.0.0.1:8081/api/v1/ws?topics=alerts,logs".to_string());
    ...
    println!("Connected successfully! (HTTP Status: {})", response.status());
    let (mut write, mut read) = ws_stream.split();

    // Send a keepalive ping
    let ping_msg = serde_json::json!({ "action": "ping" });
    write.send(Message::Text(ping_msg.to_string().into())).await?;
